...
// 删除后一律失效内存缓存（cache=null），而非预填一份 defaults()：
// 否则内存里持有的默认进度会随任意 saveMeta() 被写回该槽"复活"为非空默认档，与"删除"语义不符。
// 既然该槽在存储中已空，下次 loadMeta() 自然重建为默认开局（维持当前会话仍可玩），无需在此预载。
export function deleteSlot(n) {
  ...
  if (cacheSlot === n || getActiveSlot() === n) {
    cache = null;
    cacheSlot = null;
  }
}

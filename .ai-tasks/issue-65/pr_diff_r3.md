...
diff --git a/.ai-tasks/issue-65/ai-reviewer-prompt_r1.md b/.ai-tasks/issue-65/ai-reviewer-prompt_r1.md
new file mode 100644
index 0000000..990e77a
--- /dev/null
+++ b/.ai-tasks/issue-65/ai-reviewer-prompt_r1.md
@@ -0,0 +1,14 @@
+你是一个极其严格、甚至有些刁钻的资深代码审查员。
+这是代码提交后的【第 1 轮】审查。
+请阅读当前目录下的 .ai-tasks/issue-65/pr_diff_r1.md 文件，这是本次 PR 的代码变更。
+
...
diff --git a/.ai-tasks/issue-65/ai-reviewer-prompt_r2.md b/.ai-tasks/issue-65/ai-reviewer-prompt_r2.md
new file mode 100644
index 0000000..3283f55
--- /dev/null
+++ b/.ai-tasks/issue-65/ai-reviewer-prompt_r2.md
@@ -0,0 +1,14 @@
+你是一个极其严格、甚至有些刁钻的资深代码审查员。
+这是代码提交后的【第 2 轮】审查。
+请阅读当前目录下的 .ai-tasks/issue-65/pr_diff_r2.md 文件，这是本次 PR 的代码变更。
+
+请检查是否有 Bug、逻辑错误、安全问题或性能瓶颈。
+
+【⚠️ 重要输出格式要求】：
...
diff --git a/.ai-tasks/issue-65/fixer-feedback_r1.md b/.ai-tasks/issue-65/fixer-feedback_r1.md
new file mode 100644
index 0000000..2ff0f1f
--- /dev/null
+++ b/.ai-tasks/issue-65/fixer-feedback_r1.md
@@ -0,0 +1,9 @@
...
diff --git a/.ai-tasks/issue-65/fixer-feedback_r2.md b/.ai-tasks/issue-65/fixer-feedback_r2.md
new file mode 100644
index 0000000..096c965
--- /dev/null
+++ b/.ai-tasks/issue-65/fixer-feedback_r2.md
@@ -0,0 +1,7 @@
...
diff --git a/.ai-tasks/issue-65/fixer-prompt_r1.md b/.ai-tasks/issue-65/fixer-prompt_r1.md
new file mode 100644
index 0000000..86a127d
--- /dev/null
+++ b/.ai-tasks/issue-65/fixer-prompt_r1.md
@@ -0,0 +1,7 @@
+你是一个资深开发工程师。这是针对上一轮代码的【第 1 轮】修复任务。
+以下是代码审查员给你的修改建议，请阅读当前目录下的 .ai-tasks/issue-65/fixer-feedback_r1.md 文件获取详细内容。
+
+【⚠️ 严格红线规则】：
...
diff --git a/.ai-tasks/issue-65/fixer-prompt_r2.md b/.ai-tasks/issue-65/fixer-prompt_r2.md
new file mode 100644
index 0000000..114cde3
--- /dev/null
+++ b/.ai-tasks/issue-65/fixer-prompt_r2.md
@@ -0,0 +1,7 @@
+你是一个资深开发工程师。这是针对上一轮代码的【第 2 轮】修复任务。
+以下是代码审查员给你的修改建议，请阅读当前目录下的 .ai-tasks/issue-65/fixer-feedback_r2.md 文件获取详细内容。
+
+【⚠️ 严格红线规则】：
+请绝对不要修改、重命名或生成 .github/ 目录下的任何文件（特别是 workflows 等 CI/CD 配置），这会破坏项目的自动化流程！如果审查员要求你修改这些文件，请在回复中说明无法自动修改，需要人类介入。
+
+请直接分析并修改当前项目中的代码文件来满足审查员的要求，不要做过多的文字解释。
diff --git a/.ai-tasks/issue-65/pr_diff_r1.md b/.ai-tasks/issue-65/pr_diff_r1.md
new file mode 100644
index 0000000..4f5b312
--- /dev/null
+++ b/.ai-tasks/issue-65/pr_diff_r1.md
@@ -0,0 +1,2023 @@
+diff --git a/.ai-tasks/issue-65/ai-coder-prompt.md b/.ai-tasks/issue-65/ai-coder-prompt.md
+new file mode 100644
+index 0000000..835da1f
...
+   if (playBtn) {
diff --git a/.ai-tasks/issue-65/pr_diff_r2.md b/.ai-tasks/issue-65/pr_diff_r2.md
new file mode 100644
index 0000000..54f2804
--- /dev/null
+++ b/.ai-tasks/issue-65/pr_diff_r2.md
@@ -0,0 +1,4170 @@
+diff --git a/.ai-tasks/issue-65/ai-coder-prompt.md b/.ai-tasks/issue-65/ai-coder-prompt.md
+new file mode 100644
+index 0000000..835da1f
...
++   const playBtn = e.target.closest('.play-btn')
++   if (playBtn) {
+diff --git a/.ai-tasks/issue-65/review_error_r1.log b/.ai-tasks/issue-65/review_error_r1.log
+new file mode 100644
+index 0000000..e69de29
+diff --git a/.ai-tasks/issue-65/review_result_r1.md b/.ai-tasks/issue-65/review_result_r1.md
+new file mode 100644
+index 0000000..e6173a6
+--- /dev/null
++++ b/.ai-tasks/issue-65/review_result_r1.md
+@@ -0,0 +1,13 @@
++我已经完成了深入审查。让我整理一下我的发现。代码在功能上是合理的（155 个逻辑测试 + 73 个 DOM 测试通过，构建正常，迁移处理了旧数据，XSS 转义正确），但我发现了一个真实的、可验证的 Bug，以及几个较小的问题。
++
++DECISION: REQUEST_CHANGES
++
++COMMENT: 整体实现质量较高：数据模型重构（档案→多任务）干净，旧档（顶层 checkins）→tasks 迁移覆盖完整且单测验证，过去日二次确认逻辑（基于 diffDays<0）与今日直点区分正确，XSS 转义到位，`_findBtn` 用遍历替代选择器拼接避免 key 含特殊字符时抛错，155+73 测试通过、vite 构建通过。但存在以下问题，建议修复后再合：
++
++【主要 Bug·用户无反馈】任务改名/删除成功后的 toast 被 `render()` 立即擦除，用户看不到任何反馈。`_doTaskRename` 与 `_onTaskDelete` 都写成「先 `_toast(...)` → `_closeTaskSheet()` → `this.render()`」，而 `render()` 首行 `this.root.innerHTML=''` 会把刚 append 到旧 toast-host 的 toast 元素连同 host 一起清掉，`_toast` 里 `requestFrame` 还没触发元素就已不在 DOM。我用 jsdom 实测：点「保存」改名后立刻查询 `.toast`，结果为 `(no toast)`，确认 toast 从未显示。注意 `_onTaskNew` 里成功 toast 是放在 `render()` **之后**（`if (created) this._toast(...)`），所以新建反馈能看到——这正是同一作者在同一 PR 里写出的**不一致**，反证改名/删除的顺序写反了。同理 `_onTaskNew` 的重名分支 `this._toast('该任务已存在')` 也在 `render()` 之前被擦除。修复方式：把 `_doTaskRename` / `_onTaskDelete` / `_onTaskNew` 重名分支的 `_toast(...)` 调用移到 `this.render()` 之后（与 `_onTaskNew` 成功分支保持一致）。（附注：profile 侧 `_doRename` / `_onSheetDelete` 也有同样历史问题，可顺手一并修正。）
++
++【次要·改名预填值】`_onTaskRename` / `_onSheetRename` 用 `nameEl.textContent.replace(/\s*当前.*$/,'')` 去掉「当前」后缀来回填输入框。若任务名/昵称本身含「当前」二字（如任务名「当前进度」），正则会从名字内部截断，回填成空/错误值。建议改用原始数据源（直接从 `getTask(profile,key).name` 取）而非 DOM 文本反推。
...
+   _restoreActive() {
+     const key = getActiveKey();
+@@ -81,7 +100,26 @@ export class CheckInUI {
+     this._keyHandler = (e) => {
+       if (e.key === 'Escape') {
+         if (this._celebrating) this._endCelebrate();
++        else if (this._confirmOpen) this._closeConfirm();
...
+     };
+     window.addEventListener('keydown', this._keyHandler);
+@@ -117,7 +155,7 @@ export class CheckInUI {
+         <span class="profile-item__avatar" aria-hidden="true">${esc(firstChar(p.nickname))}</span>
+         <span class="profile-item__meta">
+           <span class="profile-item__name">${esc(p.nickname)}</span>
+-          <span class="profile-item__sub">累计 ${p.total} 天 · ${'♡'.repeat(Math.min(p.hearts, 8))}${p.hearts > 8 ? '…' : ''}</span>
...
+         <span class="profile-item__go" aria-hidden="true">›</span>
+       </button>
+@@ -128,7 +166,7 @@ export class CheckInUI {
+         <div class="launcher-hero">
+           <div class="launcher-emoji" aria-hidden="true">♡</div>
+           <h1 class="launcher-title">每日打卡</h1>
+-          <p class="launcher-sub">坚持的每一天，都开出一朵小花 🌸<br/>每累计 10 天，收获一颗爱心。</p>
...
+         <form class="launcher-form" data-id="form">
+           <input
+@@ -154,14 +192,16 @@ export class CheckInUI {
+   // ===================== 主视图 =====================
+   _renderMain() {
+     const p = this.profile;
++    const task = this._activeTask();
...
+ 
+     this.root.insertAdjacentHTML('beforeend', `
+@@ -174,6 +214,13 @@ export class CheckInUI {
+           <button class="daka-switch" data-act="sheet" type="button" title="切换 / 管理档案">档案</button>
+         </header>
+ 
++        <button class="task-bar" data-act="task-sheet" type="button" title="切换 / 管理打卡任务">
...
+           <div class="stat">
+             <span class="stat__num">${total}</span>
+@@ -222,20 +269,22 @@ export class CheckInUI {
+   _renderCalendarInner() {
+     const { year, month } = this.view;
+     const { cells } = monthMatrix(year, month, WEEK_START);
+-    const p = this.profile;
...
+       return `<button class="${cls}" data-act="day" data-iso="${iso}" type="button"${future ? ' disabled' : ''}>
+         <span class="day__num">${d.getDate()}</span>
+@@ -291,15 +340,26 @@ export class CheckInUI {
+       case 'sheet-new': this._onNewFromSheet(); break;
+       case 'sheet-rename': this._onSheetRename(btn.dataset.key); break;
+       case 'sheet-delete': this._onSheetDelete(btn.dataset.key); break;
++      case 'task-sheet': this._openTaskSheet(); break;
...
+       if (chars.length > max) el.value = chars.slice(0, max).join('');
+     }
+@@ -345,50 +405,75 @@ export class CheckInUI {
+     this.render();
+   }
+ 
+-  // —— 今日大按钮：切换今日打卡 ——
...
+     if (todayCard) {
+       todayCard.classList.toggle('is-checked', checkedToday);
+@@ -442,7 +527,26 @@ export class CheckInUI {
+     this._openSheet({ focusRename: this.profile.key });
+   }
+ 
+-  // ===================== 档案管理弹层 =====================
...
+     this._closeSheet(true);
+     const profiles = listProfiles();
+@@ -454,7 +558,7 @@ export class CheckInUI {
+             <span class="sheet-row__avatar" aria-hidden="true">${esc(firstChar(p.nickname))}</span>
+             <span class="sheet-row__meta">
+               <span class="sheet-row__name">${esc(p.nickname)}${active ? ' <em>当前</em>' : ''}</span>
+-              <span class="sheet-row__sub">累计 ${p.total} 天 · ${p.hearts} 颗爱心</span>
...
+           </button>
+           <button class="sheet-row__icon" data-act="sheet-rename" data-key="${esc(p.key)}" type="button" title="改名" aria-label="改名">✎</button>
+@@ -502,29 +606,19 @@ export class CheckInUI {
+     void silent;
+   }
+ 
+-  // 在档案弹层内按 act + 精确 key 查找按钮。
...
+     `;
+     row.appendChild(wrap);
+@@ -550,9 +644,11 @@ export class CheckInUI {
...
+ 
+   _onSheetDelete(key) {
+@@ -560,7 +656,7 @@ export class CheckInUI {
+     const p = map[normalizeKey(key)];
+     if (!p) return;
+     // 二次确认：点一次变红「确认删除」，再点一次才真删。
+-    const delBtn = this._findSheetBtn('sheet-delete', key);
...
+       delBtn.classList.add('is-armed');
+       delBtn.textContent = '确认?';
+@@ -576,9 +672,221 @@ export class CheckInUI {
...
+ 
+   // ===================== 庆祝动画（每 10 天） =====================
+@@ -659,7 +967,7 @@ export class CheckInUI {
+ }
+ 
+ // ===================== 小工具 =====================
+-// 转义 HTML，防止昵称含 < > & " 撑破 DOM / XSS。
...
new file mode 100644
index 0000000..e69de29
diff --git a/.ai-tasks/issue-65/review_error_r2.log b/.ai-tasks/issue-65/review_error_r2.log
new file mode 100644
index 0000000..e69de29
diff --git a/.ai-tasks/issue-65/review_result_r1.md b/.ai-tasks/issue-65/review_result_r1.md
new file mode 100644
...
+
+未涉及 CI/CD 或 workflows 变更，无需人工处理 GitHub Actions。
diff --git a/.ai-tasks/issue-65/review_result_r2.md b/.ai-tasks/issue-65/review_result_r2.md
new file mode 100644
index 0000000..4ae4658
--- /dev/null
+++ b/.ai-tasks/issue-65/review_result_r2.md
@@ -0,0 +1,8 @@
+DECISION: REQUEST_CHANGES
...
+// —— 名称工具（昵称 / 任务名共用规范化）——
 ...
 ok(normalizeKey('  AbC  ') === 'abc', 'normalizeKey 去首尾空白并小写');
 ok(normalizeKey('小　甜　甜') === '小_甜_甜', 'normalizeKey 全角空白也归一'); // 全角空格属 \s
@@ -165,8 +175,79 @@ ok(isValidNickname('a'.repeat(13), 1, 12) === false, '超长昵称非法');
 ok(isValidNickname('a'.repeat(12), 1, 12) === true, '12 字昵称刚好合法');
 ok(normalizeNickname('  小   甜  ', 12) === '小 甜', 'normalizeNickname 压缩空白');
 ok([...normalizeNickname('甜'.repeat(20), 12)].length === 12, 'normalizeNickname 按码点截断到 12');
+ok(isValidTaskName('跑步') === true, '合法任务名通过');
...
+// ===================== store（多档案 + 多任务）=====================
 function memStorage() {
   const m = {};
   return {
@@ -179,9 +260,10 @@ _setStorage(memStorage());
 ok(Object.keys(loadAll()).length === 0, '空存储无档案');
 ok(hasAnyProfile() === false, '空存储 hasAnyProfile=false');
...
+ok(p1.tasks && Object.keys(p1.tasks).length === 1, '新档案自带默认任务');
 ...
 ok(hasAnyProfile() === true, '有档案后 hasAnyProfile=true');
 // 同 key 再次 ensure 不重复创建，仅刷新 lastSeen
@@ -189,18 +271,21 @@ const p1Again = ensureProfile('小甜甜改名了', '小甜甜'); // 显式 key
 ok(p1Again.key === '小甜甜', '相同 key 不重复创建（沿用原档案）');
 ok(p1Again.nickname === '小甜甜', '已存在时忽略传入的 nickname');
...
+ok(typeof list[0].taskCount === 'number' && list[0].taskCount >= 1, 'listProfiles 含 taskCount 聚合');
 ...
 // renameProfile：仅改名
 ok(renameProfile('小甜甜', '甜甜') === true, 'renameProfile 成功');
@@ -221,11 +306,29 @@ ok(getActiveKey() === '甜甜', 'setActiveKey/getActiveKey 往返');
 setActiveKey(null);
 ok(getActiveKey() === null, 'setActiveKey(null) 清除激活');
...
+    nickname: '   ', key: '', tasks: null,
   ...
   upsertProfile(bad);
@@ -233,25 +336,34 @@ ok(getActiveKey() === null, 'setActiveKey(null) 清除激活');
   ok(fixed !== null, 'migrate 兜底空昵称为「未命名」');
   ok(fixed.nickname === '未命名', 'migrate 规范化昵称');
   ok(fixed.key === '未命名', 'migrate 补齐 key');
-  ok(Array.isArray(fixed.checkins) && fixed.checkins.length === 1 && fixed.checkins[0] === '2026-07-01', 'migrate 过滤非法/重复日期');
...
+  const dates = sortedDates(task);
   ...
   let best = 1;
   let run = 1;
@@ -147,10 +159,136 @@ export function longestStreak(profile) {
   return Math.max(best, run);
 }
...
+  return String(name || '')
     ...
     .replace(/\s+/g, '_')
     .toLowerCase();
@@ -169,3 +307,16 @@ export function normalizeNickname(nickname, max) {
   const chars = [...trimmed];
   return chars.slice(0, max).join('');
 }
+
...
+
   ...
   _restoreActive() {
     const key = getActiveKey();
@@ -81,7 +100,28 @@ export class CheckInUI {
     this._keyHandler = (e) => {
       if (e.key === 'Escape') {
         if (this._celebrating) this._endCelebrate();
+        else if (this._confirmOpen) this._closeConfirm();
...
+        }
     ...
     window.addEventListener('keydown', this._keyHandler);
@@ -117,7 +157,7 @@ export class CheckInUI {
         <span class="profile-item__avatar" aria-hidden="true">${esc(firstChar(p.nickname))}</span>
         <span class="profile-item__meta">
           <span class="profile-item__name">${esc(p.nickname)}</span>
-          <span class="profile-item__sub">累计 ${p.total} 天 · ${'♡'.repeat(Math.min(p.hearts, 8))}${p.hearts > 8 ? '…' : ''}</span>
...
@@ -128,7 +168,7 @@ export class CheckInUI {
         <div class="launcher-hero">
           <div class="launcher-emoji" aria-hidden="true">♡</div>
           <h1 class="launcher-title">每日打卡</h1>
-          <p class="launcher-sub">坚持的每一天，都开出一朵小花 🌸<br/>每累计 10 天，收获一颗爱心。</p>
+          <p class="launcher-sub">坚持的每一天，都开出一朵小花 🌸<br/>一个昵称下，可建多个打卡任务。</p>
         ...
         <form class="launcher-form" data-id="form">
           <input
@@ -154,14 +194,16 @@ export class CheckInUI {
   // ===================== 主视图 =====================
   _renderMain() {
     const p = this.profile;
+    const task = this._activeTask();
...
+    const progress = heartProgress(task);
     ...
     this.root.insertAdjacentHTML('beforeend', `
@@ -174,6 +216,13 @@ export class CheckInUI {
           <button class="daka-switch" data-act="sheet" type="button" title="切换 / 管理档案">档案</button>
         </header>
...
+
         <div class="stats">
           <div class="stat">
             <span class="stat__num">${total}</span>
@@ -222,20 +271,22 @@ export class CheckInUI {
   _renderCalendarInner() {
     const { year, month } = this.view;
     const { cells } = monthMatrix(year, month, WEEK_START);
-    const p = this.profile;
...
+        past ? 'is-past' : '',
       ...
       return `<button class="${cls}" data-act="day" data-iso="${iso}" type="button"${future ? ' disabled' : ''}>
         <span class="day__num">${d.getDate()}</span>
@@ -291,15 +342,26 @@ export class CheckInUI {
       case 'sheet-new': this._onNewFromSheet(); break;
       case 'sheet-rename': this._onSheetRename(btn.dataset.key); break;
       case 'sheet-delete': this._onSheetDelete(btn.dataset.key); break;
+      case 'task-sheet': this._openTaskSheet(); break;
...
@@ -345,50 +407,75 @@ export class CheckInUI {
     this.render();
   }
...
+
     ...
     if (todayCard) {
       todayCard.classList.toggle('is-checked', checkedToday);
@@ -442,7 +529,26 @@ export class CheckInUI {
     this._openSheet({ focusRename: this.profile.key });
   }
...
+  // ===================== 档案管理弹层（昵称级）=====================
   _openSheet(opts = {}) {
     this._closeSheet(true);
     const profiles = listProfiles();
@@ -454,7 +560,7 @@ export class CheckInUI {
             <span class="sheet-row__avatar" aria-hidden="true">${esc(firstChar(p.nickname))}</span>
             <span class="sheet-row__meta">
               <span class="sheet-row__name">${esc(p.nickname)}${active ? ' <em>当前</em>' : ''}</span>
-              <span class="sheet-row__sub">累计 ${p.total} 天 · ${p.hearts} 颗爱心</span>
+              <span class="sheet-row__sub">${p.taskCount} 个任务 · 累计 ${p.total} 天 · ${p.hearts} 颗爱心</span>
           ...
           <button class="sheet-row__icon" data-act="sheet-rename" data-key="${esc(p.key)}" type="button" title="改名" aria-label="改名">✎</button>
@@ -502,29 +608,19 @@ export class CheckInUI {
     void silent;
   }
...
+      <input class="sheet-rename__input" data-id="sheet-rename-input" data-key="${esc(key)}" type="text" maxlength="${NICKNAME_MAX_LEN * 2}" value="${esc(current)}" aria-label="新昵称" />
     ...
     `;
     row.appendChild(wrap);
@@ -550,9 +646,11 @@ export class CheckInUI {
       this.profile = loadAll()[normalizeKey(name)] || this.profile;
       setActiveKey(this.profile.key);
     }
-    this._toast('已改名 ♡');
...
+    this._toast('已改名 ♡');
   ...
   _onSheetDelete(key) {
@@ -560,7 +658,7 @@ export class CheckInUI {
     const p = map[normalizeKey(key)];
     if (!p) return;
     // 二次确认：点一次变红「确认删除」，再点一次才真删。
-    const delBtn = this._findSheetBtn('sheet-delete', key);
+    const delBtn = this._findBtn(this._sheetEl, 'sheet-delete', key);
     if (delBtn && !delBtn.classList.contains('is-armed')) {
       delBtn.classList.add('is-armed');
       delBtn.textContent = '确认?';
@@ -576,9 +674,221 @@ export class CheckInUI {
     if (this.profile && normalizeKey(key) === this.profile.key) {
       this.profile = null;
     }
-    this._toast('已删除档案');
...
+    void silent;
   ...
   // ===================== 庆祝动画（每 10 天） =====================
@@ -659,7 +969,7 @@ export class CheckInUI {
 }

...
diff --git a/.ai-tasks/issue-75/ai-reviewer-prompt_r1.md b/.ai-tasks/issue-75/ai-reviewer-prompt_r1.md
new file mode 100644
index 0000000..e380e1c
--- /dev/null
+++ b/.ai-tasks/issue-75/ai-reviewer-prompt_r1.md
@@ -0,0 +1,14 @@
+你是一个极其严格、甚至有些刁钻的资深代码审查员。
+这是代码提交后的【第 1 轮】审查。
+请阅读当前目录下的 .ai-tasks/issue-75/pr_diff_r1.md 文件，这是本次 PR 的代码变更。
+
...
+- 武器，怪物，地图等等再多样性一些
diff --git a/.ai-tasks/issue-75/fixer-feedback_r1.md b/.ai-tasks/issue-75/fixer-feedback_r1.md
new file mode 100644
index 0000000..5316ebd
--- /dev/null
+++ b/.ai-tasks/issue-75/fixer-feedback_r1.md
@@ -0,0 +1,31 @@
+发现一个可复现的逻辑缺陷（中等偏低危，但确属 Bug，修复极简单），另有几处可选打磨项。本意见不涉及任何 CI/CD 或 .github 改动，无需人类改 workflow。
+
...
diff --git a/.ai-tasks/issue-75/fixer-prompt_r1.md b/.ai-tasks/issue-75/fixer-prompt_r1.md
new file mode 100644
index 0000000..501fa49
--- /dev/null
+++ b/.ai-tasks/issue-75/fixer-prompt_r1.md
@@ -0,0 +1,7 @@
+你是一个资深开发工程师。这是针对上一轮代码的【第 1 轮】修复任务。
+以下是代码审查员给你的修改建议，请阅读当前目录下的 .ai-tasks/issue-75/fixer-feedback_r1.md 文件获取详细内容。
+
+【⚠️ 严格红线规则】：
...
diff --git a/.ai-tasks/issue-75/pr_diff_r1.md b/.ai-tasks/issue-75/pr_diff_r1.md
new file mode 100644
index 0000000..6ce374b
--- /dev/null
+++ b/.ai-tasks/issue-75/pr_diff_r1.md
@@ -0,0 +1,585 @@
+diff --git a/.ai-tasks/issue-75/ai-coder-prompt.md b/.ai-tasks/issue-75/ai-coder-prompt.md
+new file mode 100644
+index 0000000..2b0d668
...
diff --git a/.ai-tasks/issue-75/review_error_r1.log b/.ai-tasks/issue-75/review_error_r1.log
new file mode 100644
index 0000000..e69de29
diff --git a/.ai-tasks/issue-75/review_result_r1.md b/.ai-tasks/issue-75/review_result_r1.md
new file mode 100644
index 0000000..db62713
--- /dev/null
+++ b/.ai-tasks/issue-75/review_result_r1.md
@@ -0,0 +1,34 @@
...
+ok(document.querySelector('.xhlz-game') !== null, '着陆后进入游戏界面');
 ...
 ok(document.querySelector('.map-grid') !== null, '渲染像素地图');
 ok(document.querySelectorAll('.cell').length === GRID * GRID, `地图含 ${GRID * GRID} 个地块（实际 ${document.querySelectorAll('.cell').length}）`);
@@ -221,6 +227,71 @@ ok(document.querySelectorAll('.chapter').length === 10, '剧情页列出 10 章
 ui.closeModal();
 await sleep(5);
 
+// ---------- 7b) 装备宝箱：穿戴/丢弃后交互键须刷新，不得重放已移除实体（issue #75 回归） ----------
+// 注入一个装备宝箱到玩家相邻的安全空地块，走上去触发 offerGear 弹窗，
+// 验证选择「装备」/「丢弃」关闭后中央交互键不再停留为「拾取宝箱」、_interactMode 已重置，
+// 且再次点击交互键不会把已被 removeEntity 的宝箱再次弹出（修复前的陈旧交互态缺陷）。
+let gearChestSeq = 0;
+const hasAdjacentEnemy = (st, x, y) => [[1, 0], [-1, 0], [0, 1], [0, -1]].some(([dx, dy]) => {
+  const e = entityAt(st, x + dx, y + dy);
+  return !!(e && e.type === 'enemy');
+});
+// 选一块相邻、可走、非阶梯、无相邻敌人的空地块放宝箱，保证关闭弹窗后交互键稳定回退为「调查」。
+function placeGearChestAdjacent() {
+  const st = ui.player.floorState;
+  for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
+    const nx = st.pos.x + dx, ny = st.pos.y + dy;
+    if (nx < 0 || ny < 0 || nx >= GRID || ny >= GRID) continue;
+    if (entityAt(st, nx, ny)) continue;
+    if (!isWalkable(st.grid[ny][nx]) || st.grid[ny][nx] === 'stairs') continue;
+    if (hasAdjacentEnemy(st, nx, ny)) continue;
+    const chest = { id: `e-gear-${gearChestSeq++}`, type: 'chest', x: nx, y: ny,
+      gear: { slot: 'weapon', name: '测试光刃', stat: 999, plus: 0, affix: null } };
+    st.entities.push(chest);
+    ui.renderMap();
+    return chest;
+  }
+  return null;
+}
+async function exerciseGearChest(pickLabel) {
+  const chest = placeGearChestAdjacent();
+  if (!chest) return false;
+  ui.tryMoveTo(chest.x, chest.y); // 踩上宝箱 → resolveEntity → offerGear
+  await sleep(10);
+  if (!/拾得/.test(document.querySelector('.sheet__head .t')?.textContent || '')) return false;
+  const btn = [...document.querySelectorAll('.sheet__foot button')].find((b) => b.textContent === pickLabel);
+  if (btn) btn.click();
+  await sleep(10);
+  return true;
+}
+
+// 「装备」路径
+if (await exerciseGearChest('装备')) {
+  ok(!/拾取宝箱/.test(document.querySelector('.interact-btn')?.textContent || ''), '装备宝箱选「装备」后交互键不再停留为「拾取宝箱」');
+  ok(ui._interactMode && ui._interactMode.mode === 'investigate', '装备后 _interactMode 已重置为 investigate（不再指向已删除实体）');
+  // 关键回归：再次点交互键不得重放已移除实体的装备弹窗
+  document.querySelector('.interact-btn').click();
+  await sleep(10);
+  ok(document.querySelector('.gear-drop') === null, '再次点击交互键不会重放装备宝箱弹窗（陈旧交互态已清除）');
+  if (ui._sheet) ui.closeModal();
+} else {
+  fail++; console.error('  ✗ FAIL: 未能放置装备宝箱以测试「装备」路径');
+}
+await sleep(5);
+
+// 「丢弃」路径
+if (await exerciseGearChest('丢弃')) {
+  ok(!/拾取宝箱/.test(document.querySelector('.interact-btn')?.textContent || ''), '装备宝箱选「丢弃」后交互键不再停留为「拾取宝箱」');
+  ok(ui._interactMode && ui._interactMode.mode === 'investigate', '丢弃后 _interactMode 已重置为 investigate（不再指向已删除实体）');
+  document.querySelector('.interact-btn').click();
+  await sleep(10);
+  ok(document.querySelector('.gear-drop') === null, '丢弃后再次点击交互键不会重放装备宝箱弹窗');
+  if (ui._sheet) ui.closeModal();
+} else {
+  fail++; console.error('  ✗ FAIL: 未能放置装备宝箱以测试「丢弃」路径');
+}
+await sleep(5);
+
 // ---------- 8) 下层 ----------
...
@@ -1144,6 +1233,53 @@ export class GameUI {
     saveToSlot(this.activeSlot, this.player);
   }
...
+
   ...
   showSettings(fromLauncher) {
     const p = this.player;
@@ -1401,6 +1537,18 @@ function spentStardust(p) {
   }
   return s;
 }
+// 计算某部位在（可选）装备覆盖下的当前数值，用于装备掉落的前后对比。

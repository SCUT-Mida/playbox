...
- **浮岛探索（点击移动）**：每个浮岛是一张 16×16 的像素网格地图，角色视野仅 5×5（迷雾机制）。地图按楼层分为四种**生态**（遗迹地表 / 晶簇洞穴 / 虚空裂隙 / 星骸之核），各有专属配色、地砖纹理与点缀（苔藓、晶簇、符文、碎石等），边缘带视野暗角。点击相邻地块移动；强化「推进器」可一次走出更多步。踩上宝箱拾取、踩上陷阱触发、靠近敌人即可交战，找到下行阶梯深入下一层。
- **抉择型战斗（猜拳克制）**：敌人每回合摆出「突刺 / 横斩 / 重击」架势，玩家在限时内选择「格挡 / 闪避 / 反击」——反击克突刺、格挡克横斩、闪避克重击。成功克制触发「专注力」加成（下一击 ×1.5）。精力过低会失手。**默认开启自动战斗**由 AI 按最优克制代打（设置或战斗内可关），适合单手摸鱼；每次胜利后弹出**战利品面板**，星骸 / 零件 / 经验 / 升级一目了然。
...
> 实现注记：设计指引建议面板切换走 Hash 路由、地图用 Canvas 绘制。本作运行于落地页 overlay 内，为避免修改宿主 `window.location.hash` 造成冲突，面板切换改用应用内状态机；为兼顾 jsdom 可测性与产物体积，地图以 CSS 像素网格渲染（地块纹理 + 楼层生态着色 + 点缀层 + 视野暗角），逻辑等价且可在无头环境驱动。其余系统（默认自动的猜拳战斗 + 战利品面板、装备强化 +5 词缀、三天赋树、10 章记忆、双结局）均按方案完整实现。

...
// weeksPerTurn 统一为 WEEKS_PER_MONTH（4 周 = 1 个月）——即「一个月一个周期」，
// 让岁月流逝更从容细腻、抉择更密集；搭配挂机模式可自动推进，不必手动连点。
// 每次推进恰为整月，与日历对齐，年龄展示始终精确到月。
export const STAGES = [
  { key: 'infant', name: '婴儿期', emoji: '👶', minAge: 0, maxAge: 6, weeksPerTurn: WEEKS_PER_MONTH, desc: '嗷嗷待哺，世界充满新奇。' },
  { key: 'child', name: '学龄期', emoji: '🧒', minAge: 6, maxAge: 18, weeksPerTurn: WEEKS_PER_MONTH, desc: '入学读书，性格与天赋初现。' },
  { key: 'adult', name: '成年期', emoji: '🧑', minAge: 18, maxAge: 65, weeksPerTurn: WEEKS_PER_MONTH, desc: '工作成家，人生的主战场。' },
  { key: 'elder', name: '老年期', emoji: '🧓', minAge: 65, maxAge: Infinity, weeksPerTurn: WEEKS_PER_MONTH, desc: '颐养天年，回首这一生的得失。' },
];

export const STAGE_BY_KEY = Object.fromEntries(STAGES.map((s) => [s.key, s]));

// 每回合（= 每个月）触发随机事件的概率。
// 因现已改为月度周期，回合数大幅增加，这里把概率调低，使抉择大约「每几个月一次」，
// 既保证丰富度，又避免每个月都被事件打断、节奏过密。
...
// 事件去重冷却：最近触发过的若干事件 id 暂时不再出现（若池中还有其他可选事件），
// 缓解「每周目选择重复」的问题。数值为保留的最近 id 数量。
export const RECENT_EVENT_COOLDOWN = 8;
